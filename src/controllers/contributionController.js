...
import { getContributionGraph, generate3DContributionGrid } from '../engines/contributionEngine.js';
...
const getContributions = async (req, res) => {
  try {
    ...
    const cacheKey = `contributions_${username}_${theme}`;
    const cachedSvg = get(cacheKey);
    if (cachedSvg) {
      return res.setHeader('Content-Type', 'image/svg+xml').send(cachedSvg);
    }

    const { grid, totalContributions } = await getContributionGraph(username);
    ...
    const svg = generateContributionGraph(grid, themeObj, totalContributions);
    ...
    else if (error.message.includes('403') || error.message.includes('401')) errorMessage = 'Rate limit / auth error';
    else if (error.message.includes('GITHUB_TOKEN')) errorMessage = 'GITHUB_TOKEN not set — add it to .env';
...
const getContributions3D = async (req, res) => {
  try {
    ...
    const cacheKey = `contributions3d_${username}_${theme}`;
    const cachedSvg = get(cacheKey);
    if (cachedSvg) {
      return res.setHeader('Content-Type', 'image/svg+xml').send(cachedSvg);
    }

    const calendar = await fetchContributionGraph(username);
    ...
    const themeObj = loadTheme(theme);
    const svg = generateContribution3DGraph(grid, themeObj, totalContributions);
