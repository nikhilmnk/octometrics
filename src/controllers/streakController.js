...
export const streakController = async (req, res) => {
  try {
    ...
    let errorMessage = 'Error fetching streak';
    if (error.message.includes('404')) errorMessage = 'User not found';
      ...
      errorMessage = 'GITHUB_TOKEN not set';
    }

    const errorSvg = `<svg width="420" height="200" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="#1e1e1e" rx="8"/><text x="20" y="60" font-family="Arial" font-size="16" fill="#ff7b72">${errorMessage}</text><text x="20" y="90" font-family="Arial" font-size="12" fill="#8b949e">This widget requires GitHub contribution data.</text></svg>`;
    return res.setHeader('Content-Type', 'image/svg+xml').send(errorSvg);
  }
};
