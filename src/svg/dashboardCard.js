...
const LANG_COLORS = {
  ...
  Shell: '#89e051', Vue: '#41b883', React: '#61dafb', default: '#8b949e',
};
const lc = (lang) => LANG_COLORS[lang] || LANG_COLORS.default;

// ── STAT ICONS ────────────────────────────────────────────────────────────────
const ICONS = {
  repos: 'M2 2.5A2.5 2.5 0 014.5 0h8.75a.75.75 0 01.75.75v12.5a.75.75 0 01-.75.75h-2.5a.75.75 0 110-1.5h1.75v-2h-8a1 1 0 00-.714 1.7.75.75 0 01-1.072 1.05A2.495 2.495 0 012 11.5v-9zm10.5-1V9h-8c-.356 0-.694.074-1 .208V2.5a1 1 0 011-1h8z',
  followers: 'M5.5 3.5a2 2 0 100 4 2 2 0 000-4zM2 5.5a3.5 3.5 0 115.898 2.549 5.507 5.507 0 013.034 4.084.75.75 0 11-1.482.235 4.001 4.001 0 00-7.9 0 .75.75 0 01-1.482-.236A5.507 5.507 0 013.102 8.05 3.49 3.49 0 012 5.5zM11 4a.75.75 0 100 1.5 1.5 1.5 0 01.666 2.844.75.75 0 00-.416.672v.352a.75.75 0 00.574.73c1.2.289 2.162 1.2 2.522 2.372a.75.75 0 101.434-.44 5.01 5.01 0 00-2.56-3.012A3 3 0 0011 4z',
...
const renderStats = (stats, x, y, W, theme) => {
  ...
  return items.map((item, i) => {
    ...
    const cx = x + col * CW, cy = y + row * (CH + 6);
    return `
...
const layoutDefault = (data, theme) => {
  ...
  const W = 760, PAD = 20, COL = (W - PAD * 3) / 2;
  ...
  // Bio section
  const bioSVG = bio ? `<text x="${LEFT}" y="78" font-family="Arial,sans-serif" font-size="11" fill="${sub}">${escapeSVG(trunc(bio, 70))}</text>` : '';
...
const layoutCompact = (data, theme) => {
  ...
  const H = PAD + 50 + 14 + 100 + 20 + langH + 20 + Math.min(3, repos.length) * 42 + PAD;

  const bg = theme.background, bdr = theme.borderColor, txt = theme.textColor;
  const sub = theme.subtextColor || '#8b949e', ico = theme.iconColor || theme.accentColor;

  const bioSVG = bio ? `<text x="${PAD}" y="68" font-family="Arial,sans-serif" font-size="10" fill="${sub}">${escapeSVG(trunc(bio, 55))}</text>` : '';
...
const layoutWide = (data, theme) => {
  const { username, name, bio, stats, languages, repos } = data;
  const W = 1000, PAD = 20, COL = (W - PAD * 4) / 3;
  const langH = Math.min(6, Object.keys(languages).length) * 24 + 4;
  const repoH = Math.min(4, repos.length) * 42;
  ...
  const C1 = PAD, C2 = PAD * 2 + COL, C3 = PAD * 3 + COL * 2;
  const bioSVG = bio ? `<text x="${PAD}" y="52" font-family="Arial,sans-serif" font-size="11" fill="${sub}">${escapeSVG(trunc(bio, 90))}</text>` : '';
...
export const generateDashboardCard = (data, theme, layout = 'default') => {
  switch (layout) {
    case 'compact': return layoutCompact(data, theme);
    case 'wide': return layoutWide(data, theme);
    default: return layoutDefault(data, theme);
  }
};